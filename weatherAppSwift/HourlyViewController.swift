...
class HourlyViewController: UIViewController, UITableViewDelegate, UITableViewDataSource {
    ...
    func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        ...
        let data: WeatherModel.Data = weather!.hourly.data[indexPath.row]
        ...
        cell.firstRowTwo.text = "HM: "+String(Int(data.humidity*100))+"%"
        cell.firstRowThree.text = "DP: "+String(data.dewPoint)+"º"
        cell.firstRowFour.text = "CC: "+String(Int(data.cloudCover*100))+"%"
        cell.secondRowOne.text = "PP: "+String(Int(data.precipProbability*100))+"%"
        cell.secondRowTwo.text = "TP: "+String(data.temperature)+"º"
